...
const ProgressionTracker = () => {
  const [tasks, setTasks] = useState([]); //tasks: An array of task objects, each containing an id, name, percentage, and checked status.
  const [newTask, setNewTask] = useState("");//newTask: Stores the name of a new task being added or edited.
  const [newPercentage, setNewPercentage] = useState("");//newPercentage: Stores the completion percentage of a new task being added or edited.
  const [editingTask, setEditingTask] = useState(null); // editingTask: Tracks the task currently being edited (if any).
  ...
  return (
    <div className="flex  h-screen w-full bg-gray-900 text-white ">
      ...
      <div className=" flex w-full justify-center flex-col md:flex-row items-center bg-gray-800">
        <div className="w-full md:w-1/2 p-4 md:p-6 md:h-[480px] bg-gray-800 mt-12 rounded-lg shadow-lg">
          <h2 className="text-xl md:text-2xl font-bold mb-4">Task List</h2>
          {/* Input Section */}
          <div className="flex flex-col sm:flex-row gap-2 mb-5 md:mb-20">
            <input
              type="text"
          ...
          {/* Task List */}
          <div className="md:max-h-[300px] max-h-[200px] overflow-y-auto">
            {tasks.map((task) => (
        ...
        {/* Chart Section */}
        <div className="w-full md:w-1/2 p-4 md:p-6 mt-6 md:mt-20 bg-gray-800 rounded-lg md:h-[450px] shadow-lg flex flex-col items-center">
          <h2 className="text-xl md:text-2xl font-bold mb-20">Progress Graph</h2>
